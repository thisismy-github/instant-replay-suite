...
#
    ...
    def getItems(self, section):    # TODO FIXME: this probably doesn't work anymore
        try:
    ...
    def __getitem__(self, key):             # TODO this is a little unintuitive
        try: return self.__parser[key]
        except: return None
    ...
    def __setattr__(self, name, value):             # TODO horrific performance?
        if name[:18] != '_ConfigParseBetter':
            if type(value) in ITER_TYPES:     # TODO is this part redundant?
                delimiter = self.__defaultDelimiter
                casted_value = delimiter.join(str(v) for v in value)
            else: casted_value = str(value)
    ...
    #def __getattr__case_sensitive(self, name):
    ...
    #def __getattr__not_case_sensitive(self, name):
    #    ''' Handles getting attributes that don't exist in __dict__ yet. '''
    #    #if not name.islower(): return self.__getattribute__(name.lower())
    #    if not name.islower():
    #        name = name.lower()
    #        try: return self.__dict__[name]
    #        except KeyError: pass
...
class ConfigParseBetter(_ConfigParseBetter):
    ...
    def __setattr__(self, name, val):
        ...
        if isinstance(attr, OptionProxy): attr.set(val)
