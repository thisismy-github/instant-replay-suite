...
parser.add_argument('-i', '--ignore', nargs='*', default=None, help='File(s) within the given archive that should not be extracted')
...
# extract non-ignored files from zip file and save outcome
try:
    with ZipFile(args.zip, 'r') as zip:
        extract = zip.namelist()
        if args.ignore:
            extract = [f for f in extract if f not in args.ignore]
        zip.extractall(args.destination, members=extract)
    report_text = 'SUCCESS'
